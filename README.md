...
The API will start on `http://localhost:5242` (and `https://localhost:7087` when running the `https` launch profile). Once running, the best n stories can be requested with:

```
GET http://localhost:5242/api/stories/best?n=10
```
...
- The response currently returns the full array of n requested stories. In a real-world scenario, this would instead be paginated, for example with a page size of 25 items, to avoid large payloads and give clients more control over how much data they fetch at once.
- Instead of polling `beststories.json` and each item on a fixed interval, a future implementation could make use of the `/v0/updates` endpoint to detect which items actually changed and refresh only those, reducing unnecessary calls to the Hacker News API.
